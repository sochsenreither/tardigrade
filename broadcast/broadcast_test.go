...
func NewTestBBInstance(n, ts, sender int) *testBBInstance {
	...
	multicastFunc := func(msg *broadcastMessage) {
		for _, node := range testBBInstance.nodeChans {
			node <- msg
		}
	}

	for i := 0; i < n; i++ {
		receive := func(index int) func() chan *broadcastMessage {
			return func() chan *broadcastMessage{
				return testBBInstance.nodeChans[index]
			}
		}
		...
		testBBInstance.broadcasts[i] = NewBroadcast(n, i, ts, sender, testBBInstance.killChans[i], testBBInstance.outs[i], multicastFunc, receive(i))
	}
